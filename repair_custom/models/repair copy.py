...
class Repair(models.Model):
    ...
    def action_atelier_start(self):
        """ 
        Démarrage :
        1. Si on vient du Kiosque (tablette), on assigne l'employé du contexte (bouton 'Prendre').
        2. Si on est manager (Backend), on ne touche pas au champ technicien (il reste vide ou celui déjà choisi).
        3. On passe en 'under_repair'.
        """
        self.ensure_one()

        if self.quote_required and self.state == 'confirmed' and not self.env.context.get('force_start'):
            raise UserError("Un devis est exigé ! Veuillez faire la demande ou passer outre via le menu devis.")
        ...
        if self.env.context.get('atelier_employee_id'):
            ...
            if self.env.context.get('force_start'):
                ...
                self.message_post(body=f"🔧 <b>{tech_name}</b> a pris en charge l'intervention.")
    ...
    def action_manager_validate_quote(self):
        """ Le manager valide -> On nettoie les notifications de TOUT LE MONDE """
        self.ensure_one()
        
        # 1. On cherche toutes les activités liées à cette réparation
        # On filtre potentiellement sur le type 'Validation Devis Requise' pour ne pas fermer d'autres tâches (appels client etc)
        activities_to_close = self.env['mail.activity'].search([
            ('res_model', '=', 'repair.order'),
            ('res_id', '=', self.id),
            ('summary', '=', 'Validation Devis Requise') # Le même titre que dans le wizard
        ])
        
        # 2. On les marque toutes comme "Fait" d'un coup
        # Le feedback indiquera qui a finalement appuyé sur le bouton
        if activities_to_close:
            activities_to_close.action_feedback(feedback=f"Validé par {self.env.user.name}")

        self.message_post(body="✅ Devis validé par le management. Reprise de l'intervention.")
    ...
    def action_create_quotation_wizard(self):
        """ Ouvre le wizard de facturation en mode 'Devis' pré-rempli """
        self.ensure_one()
        
        # On calcule l'ID de la catégorie pour le wizard (comme dans action_open_pricing_wizard)
        device_categ_id = False
        if self.device_id and self.device_id.category_id:
            device_categ_id = self.device_id.category_id.id
        ...
        return {
            ...
            'context': {
                ...
                # C'EST ICI QU'ON FORCE LE MODE DEVIS :
...
class RepairQuotationWizard(models.TransientModel):
    ...
    def action_confirm_request(self):
        ...
        for manager_user in group_manager.users:
            self.repair_id.activity_schedule(
                ...
                # IMPORTANT : On utilise un titre précis qu'on pourra détecter
    ...
    def action_force_start(self):
        """ Le technicien décide de passer outre et de commencer tout de suite """
        self.ensure_one()
        
        # On appelle la fonction de démarrage du parent
        # IMPORTANT : On passe 'force_start' ET on repasse 'atelier_employee_id' qui est dans le contexte actuel
        return self.repair_id.with_context(
            force_start=True,
            atelier_employee_id=self._context.get('atelier_employee_id')
        ).action_atelier_start()
